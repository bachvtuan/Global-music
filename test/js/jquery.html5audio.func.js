...
function open_popup(url, w, h, resizable){//public api to open popup
	...
	if(!hap_popup || hap_popup.closed) {
		if(resizable){
			hap_popup=window.open(url,'name','menubar=no,toolbar=no,location=no,scrollbars=1,resizable,width='+w+',height='+h+',left='+cw+',top='+ch+'');
		}else{
			hap_popup=window.open(url,'name','menubar=no,toolbar=no,location=no,width='+w+',height='+h+',left='+cw+',top='+ch+'');
		}
...
function updatePlayerData(isPopup){
	...
	if(playlistLoaded){
		...
		if(!hidden_playlist){ 
			...
			var playlist = hap_players[0].find('#hidden_playlist_holder').children();//hidden_playlist_holder doesnt have ul node, just li nodes
			if(playlist.length > 0){
				...
				if(hap_players[0].getPlaylistList()){
					...
					hap_players[0].find('#hidden_playlist_holder').html('');//empty hidden_playlist_holder
				}
			}
		}
...
jQuery(document).ready(function(jQuery) {
	if(isMobile)jQuery('.hap_hdmb').remove();
});